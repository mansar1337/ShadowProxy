...
sing_box_cm_add_hysteria2_outbound() {
    ...
    echo "$config" | jq \
        ...
        + (if $network != "" then {network: $network} else {} end)
        + (if $hop_interval != "" then {hop_interval: $hop_interval} else {} end)
    )]'
}
...
#######################################
# Set XHTTP (SplitHTTP) transport settings for an outbound in a sing-box JSON configuration.
# Arguments:
#   config: string (JSON), sing-box configuration to modify
#   tag: string, identifier of the outbound to modify
#   path: string, HTTP path (optional)
